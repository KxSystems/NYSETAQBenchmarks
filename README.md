...
1. **In-memory query engine benchmark** — compares query execution time across the KDB-X, KDB-X SQL, Polars, DuckDB, chDB, Pandas, KDB-X Python (`pykx`), and optional Rayforce engines.
...
| `-e`, `--engines` | Comma-separated subset of engines to run. Valid values: `kdb`, `kdbxsql`, `duckdb`, `chdb`, `polars`, `pykx`, `pandas`, `rayforce`. Default: all except the optional `rayforce` engine. |
